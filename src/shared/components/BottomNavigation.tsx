...
const BottomNavigation = () => {
  ...
  return (
    <nav
      aria-label='하단 내비게이션'
      className='fixed right-0 bottom-0 left-0 z-50 mx-auto grid h-[72px] min-w-[375px] max-w-[430px] grid-cols-4 items-start border-t border-border-default bg-bg-surface shadow-[0_-4px_18px_rgba(12,45,82,0.06)]'>
      {navigationItems.map(({label, path, icon: Icon, isActive}) => {
        ...
        return (
          <NavLink
            ...
            <Icon className='w-[20px] h-[20px] shrink-0' aria-hidden='true' />
